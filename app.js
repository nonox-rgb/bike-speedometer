  ...
  if (saved) {
    try {
      ...
      if (typeof prefs.speedAlert === 'boolean') {
        ...
        if (!prefs.speedAlert) document.getElementById('speed-alert-toggle').classList.remove('on');
      }
    } catch(e) {}
  }
...
function initMap() {
  ...
  userMarker = L.marker([48.8566, 2.3522], {
    icon: makeMarkerIcon(state.accentColor)
  }).addTo(map);
...
function onGPSUpdate(pos) {
  ...
  if (!state.gpsReady) {
    ...
    document.getElementById('gps-label').textContent = 'GPS ±' + Math.round(accuracy) + 'm';
  }
...
async function fetchSpeedLimit(lat, lon) {
  try {
    const query = '[out:json][timeout:5];way(around:30,' + lat + ',' + lon + ')[highway][maxspeed];out tags 1;';
    const url = 'https://overpass-api.de/api/interpreter?data=' + encodeURIComponent(query);
    const res = await fetch(url, { signal: AbortSignal.timeout(5000) });
    const data = await res.json();
    if (data.elements && data.elements.length > 0) {
      const ms = data.elements[0].tags && data.elements[0].tags.maxspeed;
      if (ms) {
...
function updateArc(speed) {
  ...
  let color;
  if (ratio < 0.7) color = state.accentColor;
  else if (ratio < 0.9) color = '#ff6b00';
  else color = '#ff2244';
...
function renderUI() {
  ...
  document.getElementById('stat-distance').innerHTML = state.distance.toFixed(2) + '<span> km</span>';
  document.getElementById('stat-avg').innerHTML = (state.riding ? state.avgSpeed : 0).toFixed(1) + '<span> km/h</span>';
  document.getElementById('stat-max').innerHTML = state.maxSpeed.toFixed(1) + '<span> km/h</span>';
  renderSpeedLimitWarning();
}
...
function toggleWakeLock(el) {
  ...
  if (!state.wakeLockOn && state.wakeLock) { state.wakeLock.release(); state.wakeLock = null; }
  savePrefs();
}
...
function setColor(hex) {
  ...
  if (userMarker) userMarker.setIcon(makeMarkerIcon(hex));
  savePrefs();
}
